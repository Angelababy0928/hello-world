...
import MackTwo from '../compoent/mackTwo';
import shezhi from '../compoent/shezhi.png';
import fh from '../compoent/fh.png';
...
export default class Container extends Component{
    constructor(props) {
        super(props);
        this.changeIndex = this.changeIndex.bind(this);//改变学生
        this.showMask = this.showMask.bind(this);//显示弹出框
        this.cancelMask = this.cancelMask.bind(this);//关闭弹出框
        this.changeScore = this.changeScore.bind(this);//修改分数
        this.changeColor = this.changeColor.bind(this);
        this.changeColorsNext = this.changeColorsNext.bind(this);
        this.changeTwo = this.changeTwo.bind(this);//第二张页面
        this.selectMask = this.selectMask.bind(this);
        this.scroll = this.scroll.bind(this);//滚动条监听
        this.clickThree = this.clickThree.bind(this);//第三张页面
        this.state={
            data:aa,
            index:0,//学生索引，确定哪一个学生
            flag:false,//控制弹出框
            num:"",
            num1:"",
            oparateRecord: false,
    ...
    if(this.props.location.pathname=="/student/repos"){
        children = React.cloneElement(this.props.children,{//第一个页面
            id:this.state.flag,
            class:this.state.index,
        ...
        children = React.cloneElement(this.props.children, {//第二个页面
            students: this.state.data.students,
            showMask: this.showMask
        })
    }else if(this.props.location.pathname=="/student/three"){
        children = React.cloneElement(this.props.children, {//第三个页面
            id:this.state.flag,
            class:this.state.index,
            rewards:this.state.data.rewards,
            students:this.state.data.students,
    ...
    return(
        <div onScroll={this.scroll} ref="box" style={{width:"100%",height:"100%",overflow:"auto"}}>
            <Header
                ...
                img={this.props.location.pathname=="/student/repos"||this.props.location.pathname=="/student/three"?shezhi:fh}
            />
            {
                this.selectMask()
            }
            <div className="box">
                <div className="dbai" style={this.state.data.students[this.state.index].status==2?{display:"block"}:{display:"none"}}></div>
                {
                    children
                }
    ...
    selectMask(){
        if(!this.state.flag){
            ...
            if(this.props.location.pathname=="/student/repos"||this.props.location.pathname=="/student/index"){
                return <Mask flag={this.state.flag}
                             cancelMask={this.cancelMask}//取消弹出框
                             changeScore={this.changeScore}//确定改分
                             rewards={this.state.data.rewards}
                             pathName={this.props.location.pathname}
                />
            }else{
